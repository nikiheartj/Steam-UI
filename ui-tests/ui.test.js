...
test("Change language", async ({ page, baseURL }) => {
  ...
  await test.step("Wait for applying language to the page", async () => {
...
test("Go to special game category", async ({ page, baseURL }) => {
  const steam = new Steam(page);
...
test("Search game via search bar", async ({ page, baseURL }) => {
  ...
  await test.step(`Expected Result: ${game} is found`, async () => {
    await expect(steam.mainPage.selectGameFromSearchList(game)).toHaveText(
      game
    );
  });
});
